...
```
通过字段映射，可以将id转换为userId,loginId转换为loginName。这里仅需要描述一个对应关系，接口层自动完成该步骤
...
```
通过数据转换，此示例将 "status":"1" 转换为 "status":"正常"
